...
// `File` is a regular volt struct passed by value at the API surface;
// methods are defined on `File` so they fire on the binding (per v1
// decisions: Drop auto-close + explicit Close, idempotent via the
// `closed` flag).
...
fun Getenv(name string) string { ret "" }
...
fun MkdirAll(path string, mode int) int {
    ...
    var end int = n
    for end > 1 {
        if path[end-1] != 47 { break }
        end = end - 1
    }
    if Exists(path) { ret 0 }
    for i := 1; i <= end; i++ {
        var isSlash bool = false
        if i == end {
            isSlash = true
        } else {
            if path[i] == 47 { isSlash = true }
        }
        if !isSlash { continue }
